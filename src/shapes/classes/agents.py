...
from shapes.classes.shapes2D import Shapes2D
from shapes.classes.shapes3D import Shapes3D
...
class Agent:
    ...
    def __init__(
        ...
        measures: Union[dict[str, float | Sex], AgentMeasures],
        shapes2D: Optional[Shapes2D | ShapeDataType | None] = None,
        shapes3D: Optional[Shapes3D | dict[float, ShapeType | MultiPolygon] | None] = None,
    ) -> None:
        """
        ...
        if self._shapes2D.shapes:
            ...
            self._measures.measures["moment_of_inertia"] = fun.compute_moment_of_inertia(
                np.array(self._shapes2D.get_geometric_shape().exterior.coords),
    ...
    def _initialize_shapes3D(
        self,
        agent_type: cst.AgentTypes,
        shapes3D: Optional[Shapes3D | dict[float, ShapeType | MultiPolygon]],
    ) -> Shapes3D:
        """
