...
from src.shapes_package.utils.typing_custom import BackupDataType, Sex, ShapeDataType
...
def create_slider(
    label: str, min_val: float | int, max_val: float | int, value: float | int, step: int | float, key: str
) -> float | int:
    """
...
def main() -> None:
    ...
    if agents:
        ...
        current_agent.x_translation = create_slider(
            "X-Translation (cm)", -100.0, 100.0, getattr(current_agent, "x_translation", 0.0), 1.0, f"x_translation_{selected_id}"
        )

        current_agent.y_translation = create_slider(
            "Y-Translation (cm)", -100.0, 100.0, getattr(current_agent, "y_translation", 0.0), 1.0, f"y_translation_{selected_id}"
        )
