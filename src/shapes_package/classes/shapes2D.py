...
from src.shapes_package.classes.initial_agents import InitialBike, InitialPedestrian
from src.shapes_package.classes.measures import AgentMeasures
from src.shapes_package.utils import functions as fun
from src.shapes_package.utils.typing_custom import ShapeDataType
...
class Shapes2D:
    ...
    def __post_init__(self) -> None:
        ...
        -----
            ...
            - The reference direction must be within the range (-180.0, 180.0].
            If not, it will be wrapped and an error will be raised.
    ...
    def create_shape(self, name: str, shape_type: str, young_modulus: float, **kwargs: Any) -> None:
        ...
        shape_type : str
            The type of the shape. Must be one of 'circle', 'rectangle', or 'polygon'.
        young_modulus : float
        ...
        **kwargs: Any
        ...
        For polygon:
            points : list of tuples
                ...
                Must have at least 3 points and the first/last points must match.
    ...
    def get_shape(self, name: str) -> Polygon:
        ...
        -------
        Polygon
            The shapely object corresponding to the shape with the given name.

        Raises
        ------
        KeyError
            If no shape is found with the given name.
    ...
    def create_pedestrian_shapes(self, measurements: AgentMeasures) -> None:
        ...
        def objectif_fun(scaling_factor: NDArray[np.float64]) -> float:
            ...
            scaling_factor : NDArray[np.float64]
                A numpy array containing the scaling factors for x and y dimensions.

            Returns
            -------
            float
                The calculated penalty based on the difference between the new and old
                measurements of chest depth and bideltoid breadth.
    ...
    def create_bike_shapes(self, measurements: AgentMeasures) -> None:
        ...
        ----------
        measurements : AgentMeasures
            An object containing the measurements for the bike parts.

        Raises
        ------
        ValueError
            If the agent type is not a bike.
