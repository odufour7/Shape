...
def display_body3D_orthogonal_projection(agent: Agent, extra_info: tuple[DeltaGenerator, DeltaGenerator]) -> Figure:
    ...
    sm = plt.cm.ScalarMappable(
        cmap="coolwarm",
        norm=Normalize(vmin=0, vmax=float(max(agent.shapes3D.shapes.keys(), key=float))),
    )
    ...
    min_height = float(min(agent.shapes3D.shapes.keys(), key=float))
    max_height = float(max(agent.shapes3D.shapes.keys(), key=float))
    # Plot each polygon at different heights
    for height_str in sorted(agent.shapes3D.shapes.keys(), key=float):
        multi_polygon = agent.shapes3D.shapes[height_str]
